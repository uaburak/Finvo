...
struct TransactionListView: View {
    ...
    // Optional initial filter when navigating from Dashboard
    var initialFilter: TransactionType?
    ...
    var body: some View {
        ...
        .searchable(text: $viewModel.searchText, placement: .navigationBarDrawer(displayMode: .automatic), prompt: "Ara")
        .onAppear {
            if let initialFilter = initialFilter {
                viewModel.updateFilter(initialFilter)
            } else if let filter = tabManager.pendingTransactionFilter {
                viewModel.updateFilter(filter)
                tabManager.pendingTransactionFilter = nil
            }
